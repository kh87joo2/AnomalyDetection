...
function renderTabs() {
  ...
  state.dashboard.views.forEach((item, index) => {
    ...
    button.addEventListener("click", () => {
      ...
      state.transform = { x: 80, y: 50, scale: 0.9 };
      renderTabs();
      renderView();
      applyTransform();
...
async function init() {
  try {
    ...
    titleEl.textContent = state.dashboard.meta.title;
    subtitleEl.textContent = state.dashboard.meta.subtitle;
    ...
    setStatus("Ready. Drag nodes, pan, or zoom.");
  } catch (error) {
