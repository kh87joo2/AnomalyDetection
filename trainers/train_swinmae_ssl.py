...
def main() -> None:
    ...
    writer.close()
    csv_path, png_path = save_loss_history(stream="swinmae", history=history)
    print(f"[SwinMAE] loss history csv: {csv_path}")
    print(f"[SwinMAE] loss curve png: {png_path if png_path is not None else 'skipped (matplotlib not available)'}")
